...
def arrow_to_processed_pandas(table, julia_uuid_version=5):
    ...
    for schema_field in table_schema.names:
        ...
        if 'ARROW:extension:name' in metadata.keys() and metadata['ARROW:extension:name'] == 'JuliaLang.UUID':
            convert_uuid = lambda x : convert_julia_uuid_bytestring_to_uuid(x, version=julia_uuid_version)
            dataframe[schema_field] = dataframe[schema_field].map(convert_uuid)
