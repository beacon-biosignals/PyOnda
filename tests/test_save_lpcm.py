...
def test_save_array_to_lpcm_file(sample_type, n_channels, expected_eeg_data, tmpdir):
    ...
    saved_data = load_array_from_lpcm_file(
        tmpdir / "test_array.lpcm", sample_type, n_channels, "C"
    )
    assert np.array_equal(saved_data, expected_eeg_data)


def test_save_array_to_lpcm_zst_file(
    sample_type, n_channels, expected_eeg_data, tmpdir
):
    save_array_to_lpcm_zst_file(expected_eeg_data, tmpdir / "test_array.lpcm.zst")
    saved_data = load_array_from_lpcm_zst_file(
        tmpdir / "test_array.lpcm.zst", sample_type, n_channels, "C"
    )
    assert np.array_equal(saved_data, expected_eeg_data)
...
def test_save_array_to_lpcm_file_in_s3(s3, sample_type, n_channels, expected_eeg_data):
    ...
    saved_data = load_array_from_lpcm_file_in_s3(
        "s3://mock-bucket/test_array.lpcm", sample_type, n_channels, "C"
    )
    assert np.array_equal(saved_data, expected_eeg_data)


def test_save_array_to_lpcm_zst_file_in_s3(
    s3, sample_type, n_channels, expected_eeg_data
):
    save_array_to_lpcm_zst_file_in_s3(
        expected_eeg_data, "mock-bucket", "test_array.lpcm.zst"
    )
    saved_data = load_array_from_lpcm_zst_file_in_s3(
        "s3://mock-bucket/test_array.lpcm.zst", sample_type, n_channels, "C"
    )
    assert np.array_equal(saved_data, expected_eeg_data)
